...
<div class="max-w-4xl mx-auto pb-20">
    <div class="no-print bg-slate-900 text-white p-6 rounded-[30px] mb-8 shadow-xl flex justify-between items-center">
        <div>
            <p class="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-1">Generador de Registro</p>
            <h1 class="text-xl font-black italic uppercase">Modelo Oficial de Jornada</h1>
        </div>
        <button onclick="window.print()" class="bg-blue-600 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest">Imprimir</button>
    </div>

    <!-- FILTROS -->
    <div class="bg-white p-6 rounded-[30px] border border-slate-200 mb-10 no-print">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <input type="hidden" name="p" value="informe_legal">
            <select name="user_id" class="w-full bg-slate-50 border p-4 rounded-2xl font-bold text-xs uppercase" required>
                <option value="">-- Empleado --</option>
                <?php foreach($usuarios as $u): ?>
                    <option value="<?php echo $u['id']; ?>" <?php echo ($userId == $u['id'])?'selected':''; ?>><?php echo $u['nombre']; ?></option>
                <?php endforeach; ?>
            </select>
            <select name="mes" class="w-full bg-slate-50 border p-4 rounded-2xl font-bold text-xs uppercase">
            ...
            <input type="number" name="anio" value="<?php echo $anio; ?>" class="w-full bg-slate-50 border p-4 rounded-2xl font-bold">
    ...
    <div class="bg-white p-12 border border-slate-200 print:border-none print:p-0">
        <div class="flex justify-between items-start border-b-4 border-slate-900 pb-8 mb-10">
            <div>
                ...
                <p class="text-[10px] font-black text-slate-400 uppercase">Cumplimiento Legal RD-Ley 8/2019</p>
            </div>
        ...
        <div class="grid grid-cols-2 gap-10 mb-10 text-sm">
            <div>
                <p class="text-[9px] font-black text-slate-400 uppercase">Empresa Responsable</p>
                <p class="font-black text-slate-800"><?php echo EMPRESA_NOMBRE; ?></p>
                <p class="text-slate-500 italic">CIF: <?php echo EMPRESA_CIF; ?></p>
                <p class="text-slate-400 text-xs mt-1"><?php echo EMPRESA_DIRECCION; ?></p>
            </div>
            <div class="text-right">
                <p class="text-[9px] font-black text-slate-400 uppercase">Trabajador/a</p>
                <p class="font-black text-slate-800 uppercase italic"><?php echo $usuario_nombre; ?></p>
                <p class="text-slate-500 italic">Periodo: <?php echo date('F Y', mktime(0,0,0,$mes,1,$anio)); ?></p>
            </div>
        </div>
        ...
        <table class="w-full text-left border-collapse border border-slate-900">
            ...
            <tbody class="text-[11px] font-bold">
                ...
                <tr class="<?php echo isset($datos[$fecha])?'bg-white':'bg-slate-50 text-slate-300'; ?>">
                    ...
                    <td class="p-1 border border-slate-200">---</td>
                    <td class="p-1 border border-slate-200 text-right font-black text-slate-800"><?php echo $h_t; ?></td>
                </tr>
                <?php endfor; ?>
            </tbody>
            <tfoot>
                <tr class="bg-slate-100 font-black">
                    <td colspan="4" class="p-4 border border-slate-900 text-right uppercase text-[9px]">Cómputo Total Mensual</td>
                    <td class="p-4 border border-slate-900 text-right text-base"><?php echo floor($totalS/3600); ?>h <?php echo floor(($totalS/60)%60); ?>m</td>
                </tr>
            </tfoot>
        </table>
        ...
        <div class="mt-20 grid grid-cols-2 gap-20">
            <div class="border-t border-slate-400 pt-4 text-center">
                <p class="text-[9px] font-black uppercase text-slate-400">Sello de Empresa / Firma</p>
            </div>
            <div class="border-t border-slate-400 pt-4 text-center">
