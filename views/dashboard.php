...
if ($esAdmin) {
    ...
    // Contamos solo empleados reales (Excluimos a Carmen, Jaime y Remi del conteo)
    ...
    // Cálculo Horas Reales (Filtrado por rol empleado si es 'all')
    if($filtroEmp == 'all') {
        $sqlR = "SELECT f.fecha_hora, f.tipo FROM fichajes f JOIN usuarios u ON f.usuario_id = u.id 
                 WHERE MONTH(f.fecha_hora) = ? AND YEAR(f.fecha_hora) = ? AND u.rol = 'empleado'";
