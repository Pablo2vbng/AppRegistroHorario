...
<!-- DRAWER MÓVIL -->
<div id="mobile-drawer" class="fixed inset-y-0 right-0 w-72 bg-slate-900 text-white z-[120] transform translate-x-full shadow-2xl md:hidden flex flex-col font-black text-[10px] uppercase tracking-widest">
    <div class="p-6 border-b border-slate-800 flex justify-between items-center">
        <span>Menú CVTools</span>
    ...
    <nav class="flex-1 p-4 space-y-4">
        ...
        <?php endif; ?>
        <a href="logout.php" class="block p-3 border-t border-slate-800 text-rose-500">Cerrar Sesión</a>
    </nav>
</div>
...
<div class="flex">
    ...
    <aside class="hidden md:flex flex-col w-64 bg-slate-900 text-slate-300 min-h-screen sticky top-0 shadow-2xl flex-shrink-0">
        <div class="p-8 border-b border-slate-800"><img src="assets/img/logoCvTools.jpg" class="w-full rounded-xl shadow-lg border border-slate-700"></div>
        <nav class="flex-1 px-4 space-y-1 pt-6 overflow-y-auto no-scrollbar">
            <p class="px-4 py-2 text-[10px] font-black text-slate-500 uppercase tracking-widest">Principal</p>
            ...
            <?php if($_SESSION['rol'] != 'admin'): ?>
                ...
                <a href="index.php?p=solicitudes" class="flex items-center p-3 rounded-xl hover:bg-slate-800 transition"><i class="fas fa-umbrella-beach mr-3 text-amber-500"></i> Mis Vacaciones</a>
            <?php else: ?>
                <p class="px-4 py-3 text-[10px] font-black text-slate-500 uppercase tracking-widest mt-6 border-t border-slate-800 pt-6">RRHH Carmen</p>
                <a href="index.php?p=gestion_ausencias" class="flex items-center justify-between p-3 rounded-xl hover:bg-slate-800 transition text-sky-300">
                    <div><i class="fas fa-tasks mr-3"></i> Validar Solicitudes</div>
                ...
                <a href="index.php?p=informe_global" class="flex items-center p-3 rounded-xl hover:bg-slate-800 transition"><i class="fas fa-file-export mr-3"></i> Informe Anual</a>
