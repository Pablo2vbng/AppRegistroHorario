...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Evitar fallos de float por la configuración regional del servidor
    ...
    try {
        // CORRECCIÓN APLICADA AQUÍ: Se ha cambiado "fecha_registro" por "fecha_hora"
        $sql = "INSERT INTO fichajes (usuario_id, tipo, latitud, longitud, fuera_rango, ip_registro, fecha_hora) 
                VALUES (?, ?, ?, ?, ?, ?, NOW())";
        
        $stmt = $pdo->prepare($sql);
        
        // Limpiar la IP (Hostinger a veces concatena IPs, cogemos la primera)
